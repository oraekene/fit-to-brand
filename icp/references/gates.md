...
1. Header gate — every artifact opens with `project | run-mode | form | overlay | theme (broad/sharp, from S1) | model | temp | date | counts | spec-sha` (S0 additionally declares line-cap/K-rank/K-fit quotas; S3 additionally states base currency/year/PPP source). Missing header is red; missing optional platform-role is pass; undeclared quotas take defaults, silently mixed quota values are red. S0 `spec-sha` must resolve in `SOURCES.log` (E-sources) — dangling sha is red.
...
6. Honesty gate — S0 Out-of-Scope empty red; S5A NOT-fit empty without written justification red; safety-critical fit without safety note red. Unfillable section invented instead of halt-and-ask red.
...
8. Sources gate (E-sources) — missing SOURCES.log red; zero `S<nn> =` lines red; family tag outside F1-F6 red.
9. Ask gate (E-ask) — S0 exit requires Q0.0-Q0.8 + Q1.1-Q1.4 + QN.0; missing key red, never defaulted silently.
